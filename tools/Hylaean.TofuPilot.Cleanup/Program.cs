...
// Entity-name prefix to match. Pass as the first argument; defaults to the CI test-fixture prefix.
var prefix = args.Length > 0 && !string.IsNullOrWhiteSpace(args[0]) ? args[0] : "tfcs-";
...
static async Task<List<T>> CollectAll<T>(Func<double?, Task<Hylaean.TofuPilot.Models.Common.PaginatedResponse<T>>> fetcher)
{
    var all = new List<T>();
    double? cursor = null;
    do
